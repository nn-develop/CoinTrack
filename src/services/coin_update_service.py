...
class CoinUpdateService:
    ...
    async def update_coin_data(self, coin_id: str, coin_data: CoinUpdate) -> CoinUpdate:
        try:
            # Check Redis cache
            cached_data: dict[str, str] = self.redis_cache.get_coin_from_cache(coin_id)
            if cached_data:
                ...
                api_data: dict = self.coingecko_api.get_coin_info(coin_id)
                if api_data:
                    if (
                        api_data.get("symbol")
                    ...
                    if coin_data.symbol and coin_data.name:
                        self.redis_cache.set_coin_cache(
                            coin_id, coin_data.symbol, coin_data.name
                        )
                    return coin_data
        except ValueError as e:
            raise ValueError(f"Coin ID {coin_id} not found in cache or CoinGecko API")
